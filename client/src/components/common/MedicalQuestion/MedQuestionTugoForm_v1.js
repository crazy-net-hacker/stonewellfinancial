...
const MedQuestionTugoForm = (props) => {
  ...
  const handleSubmit = async (values) => {
    ...
    for (const i in values) { 
      for (const item in values[i].answer){
        ...
        if(values[i].answer[item].add_surcharge && values[i].answer[item].answer_value === true){
          ...
          if (values[i].answer[item].answer_code==='q3a2' || values[i].answer[item].answer_code==='q3a3'){
    ...
    planMedQuest.map(m => m.medicalQuestion.surcharge = parseFloat(surcharge.toFixed(0)))
