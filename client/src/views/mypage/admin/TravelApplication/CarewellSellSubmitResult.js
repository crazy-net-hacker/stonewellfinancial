...
import { sellCarewell } from '../../../../redux/actions/travelApplicationAction';
//custom component
...
const CarewellSellSubmitResult = (props) => {
  ...
  const sellCarewellResult  = useSelector((state) => state.travelApplicationReducer.sellCarewellResult)
  const sellCarewellLoading = useSelector(state => state.travelApplicationReducer.sellCarewellLoading)
  const sellCarewellError = useSelector(state => state.travelApplicationReducer.sellCarewellError)
  // console.log(sellCarewellResult)

  useEffect(()=>{
      dispatch(sellCarewell({application_id:id, data: formData})) 
  ...
  return(
    <Grid container style={{ margin: '2vh 0 3vh 0' }} spacing={3}>
      ...
      {!sellCarewellError && !sellCarewellLoading && (sellCarewellResult.status === 'success') &&
        <>
          <Grid item xs={12}>
              Result from Carewell
              {/* {sellCarewellResult.map((r, rIndex) => ( */}
                <React.Fragment>
                  <Grid item xs={12} style={{marginTop:'1vh'}}>
                    <Alert severity= {sellCarewellResult.status} >
                      { sellCarewellResult.message === 'Sell' 
                        ? <AlertTitle>{sellCarewellResult.message} - {sellCarewellResult.status}</AlertTitle>
                        : <AlertTitle>{sellCarewellResult.message} </AlertTitle>
                      }
                      {/* {r.status === 'error'
            ...
            {sellCarewellResult.status === 'error'&&
              <Alert severity={sellCarewellResult.status} >
                <AlertTitle>{sellCarewellResult.status}</AlertTitle>
                {sellCarewellResult.message }
              </Alert>
            }
          ...
          <Grid container justify="center" >
            <Button
              ...
              onClick={() => {
                var sellingResult = '' 
                if (sellCarewellResult.length > 0 && sellCarewellResult.filter(f=>f.message==='Sell').length > 0){
                    sellingResult = (sellCarewellResult.filter(f=>f.message==='Sell').map(r=>r.status)).filter(f=>f==='success').length > 0 
                                      ? 'success' : 'error';
                }                 
