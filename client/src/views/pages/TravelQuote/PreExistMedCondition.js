...
// export default () => {
const PreExistMedCondition = (props) => {
                        ...
                        <Grid item xs={12} sm={10} md={10} lg={8} xl={6} style={{ marginBottom:'5vh' }}>
                            <Typography variant="h5" gutterBottom className={classes.title_question}>
                                <span className={classes.title_question_sub}> <Text tid={'Quote.LetsFindBestPlan'}/></span>
                                    <Text tid={'Quote.PreExistingConditionQuestion'}/>
                            </Typography>
                            <Alert severity="info">
                                <AlertTitle><Text tid={'Quote.WhatIsPreExistingCondition'}/></AlertTitle>
                                    <Text tid={'Quote.PreExistingConditionIs'}/>
                            </Alert>
                        </Grid>

                        {values.insuredPersons && values.insuredPersons.filter(person => (person.age < 60)).length > 0 && (
                            values.insuredPersons.filter(person => (person.age < 60)).map((insuredPerson, i) => (
                                <Grid item container justify="center" spacing={3} key={i} style={{ marginBottom:'0' }}>
                                    ...
                                    <Grid item xs={6} md={5} lg={4} xl={3}>
                                        <ToggleButtonGroup
                                            name ={`insuredPersons.${i}.preExistCond`}
                                            value={insuredPerson.preExistCond}
                                            exclusive
                                            onChange={(e) => {
                                                const val = e.currentTarget.value === 'true' ? true : false
                                                // insuredPerson.preExistCond = val
                                                setFieldValue(`insuredPersons.${i}.preExistCond`, val)
                                            }}
...
// ProtoTypes
PreExistMedCondition.propTypes = {
  formData: PropTypes.object.isRequired
};

export default PreExistMedCondition;