...
// export default () => {
const MaternityCoverQuestion = (props) => {
    ...
    return (
        ...
        <Form className={classes.formWrapper}>
            <Grid container spacing={2} justify="center">
                <Grid item xs={12} className={classes.input_box}>
                    <Typography variant="h5" gutterBottom className={classes.title_question}>
                        <span className={classes.title_question_sub}> <Text tid={'Quote.LetsFindBestPlan'}/></span>
                            <Text tid={'Quote.MaternityCoverQuestion'}/>
                    </Typography>

                    {values.insuredPersons && values.insuredPersons.length > 0 && (
                        values.insuredPersons.filter(person => (person.gender === 'female' && person.age > 19 && person.age < 51)).map((insuredPerson, i) => (
                            <Grid container justify="center" spacing={3} key={i} style={{ marginBottom:'0' }}>
                                ...
                                <Grid item xs={6} md={6} >
                                    <ToggleButtonGroup
                                        value={insuredPerson.maternity}
                                        exclusive
                                        onChange={(e) => {
                                            const val = e.currentTarget.value === 'true' ? true : false                            
                                            insuredPerson.maternity = val
                                            setFieldValue(`insuredPerson.${i}.maternity`, val)
                                        }}
                                    >
                                    ...
                                    {/* {validMessage(`insuredPersons.${index}.maternity`)} */}
                                </Grid>
...
// ProtoTypes
MaternityCoverQuestion.propTypes = {
  formData: PropTypes.object.isRequired
};

export default MaternityCoverQuestion;
