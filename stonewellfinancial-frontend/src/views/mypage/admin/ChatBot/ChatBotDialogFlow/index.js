...
const ChatBotDialogFlow = (props) => {
  ...
  const df_event_query = useCallback(async(event, answer) => {
    const res = await axios.post('/api/v1/chatbot_dialogflow/df_event_query',  {event: event});
...
const df_text_query = async(text) => {
  if (text){
    ...
    const res = await axios.post('/api/v1/chatbot_dialogflow/df_test_query',  {text: queryText});
