...
const StepHeader = ({ location: { pathname }, height, activeStep }) => {
  ...
  const isProductSelectionStep = pathname === '/life-insurance/quote/product-selection'; 
  const isPesonalInfoStep = pathname === '/life-insurance/quote/personal-info';
  const isReviewStep = pathname === '/life-insurance/quote/review'; 
                          ...
                          <div className={`${isProductSelectionStep ? 'step active' : 'step'}`}>
                            <div>
                              {isPesonalInfoStep || isReviewStep 
                                ? (
                                <Link to="/life-insurance/quote/product-selection"><Text tid={`Quote.Step.${label}`}/></Link>
                              ) : (
                                <Text tid={`Quote.Step.${label}`}/>
                              )}
                            </div>
                        ...
                        <div className="steps">
                          <div className={`${isPesonalInfoStep ? 'step active' : 'step'}`}>
                            <div>
                              {isReviewStep 
                                ? (
                                <Link to="/life-insurance/quote/personal-info"><Text tid={`Quote.Step.${label}`}/></Link>
                              ) : (
                                <Text tid={`Quote.Step.${label}`}/>
                              )}
                            </div>
