    ...
    try{
        ...
        if (searchResult.rowCount === 0) {
            let sql = `insert into application ( application_date, insured_type, travel_direction_type, insured_group_type,
                                                  ...
                                                  contact_name, 
                                                  mailling_in, email, phone, app_status, 
                                          ...
                                          values ('${currentDate}', '${data.insuredPersons[0].eligilbeIns}', '${data.tripDirection}', '${data.insuredGroupType}',
                                                  ...
                                                  ${data.maillingInCanada}, '${data.contactEmail}', '${data.contactPhone}', '${data.status?data.status:'Pending'}', 
                                                  '${data.vendorID?data.vendorID:'VEN0000010'}', '${data.sourceFrom?data.sourceFrom:'O'}', ${data.renewal}, '${data.sourceChnnel}', '${data.preferLanguage?data.preferLanguage:'ko'}', '${data.userID}'
                                                  ) returning * `
...
const applicationSqlStatement_zcrm = () => {
    let sql = `select to_char(app.application_date,'YYYY-MM-DD') app_date, 
                      ...
                      source_from ,  source_chnnel , prefer_language, confirmation_no,   
...
const applicationSqlStatement = () => {
  let sql = `select to_char(app.application_date,'YYYY-MM-DD') app_date, 
                    ...
                    insurance_plan_price , medical_charged_price , deduct_amount, insurance_amount, option_plan_price,  carewell_service_price,
                    family_plan_amount , family_plan_discount , insurance_card_fee , total_amount,
                    app.mailling_in, app.contact_name , app.email, app.phone,  renewal , app.source_from ,  app.source_chnnel , app.prefer_language, app.confirmation_no,     
                    (select  COALESCE(json_agg(
                                        json_build_object(
